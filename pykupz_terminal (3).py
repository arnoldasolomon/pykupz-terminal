...
# ─────────────────────────────────────────────────────────────────────────
# ████████████  CHART BUILDERS  ████████████
# KEY FIX: Never use **CHART dict. Always call base_layout() + update_xaxes/yaxes.
# ─────────────────────────────────────────────────────────────────────────
...
def fig_financial_lines(ticker):
    ...
    LEFT  axis : Revenue ($B), EBITDA ($B), FCF ($B)  — bar + line
    ...
    All on same time axis so you can see exact correlation.
    """
...
def main():
    ...
    with tab1:
        ...
        with fc1: uni  = st.radio("Universe",["Q1 2026","All STB","Combined"],horizontal=True,label_visibility="collapsed")
        with fc2: topn = st.slider("Top N",10,40,25,label_visibility="collapsed")
        with fc3: srt  = st.selectbox("Sort",["Score","Change %","Market Cap","P/E"],label_visibility="collapsed")
    ...
    with tab2:
        ...
        if "Full Financial" in vmode:
                    ...
                    if avail:
                        ...
                        def fmt_cell(x):
                            ...
                            try:
                                ...
                                return str(x)
                        df_raw = df_raw.applymap(fmt_cell)
            ...
            cp = st.select_slider("Period",["1mo","3mo","6mo","1y","2y","5y","max"],value="1y",label_visibility="collapsed")
            ...
            sel = st.multiselect("Tickers", all_t, default=Q1_2026[:14], label_visibility="collapsed")
            if sel:
    ...
    with tab6:
        ...
        with cc1:
            ...
            lookup = st.text_input("", placeholder="Any ticker: NVDA, TSLA, MSFT...", label_visibility="collapsed")
            if lookup:
...
# 2. Save as pykupz_terminal.py
# 3. streamlit run pykupz_terminal.py
# 4. Opens automatically — fully live, no Excel needed
# 5. Deploy: push BOTH files to GitHub → share.streamlit.io → Deploy
# ═══════════════════════════════════════════════════════════════════════════
